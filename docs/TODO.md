...
- ☐ **M3-8** Named scent models — build (post-M4 or when inputs arrive; NOT inside M4) — C, approve I at the PRD gate. **BLOCKED on Alon inputs** (kernel as 25 numbers reconciled vs book fig.4; event-level 3-turn cadence trace; rounding + ρ provenance; their lock's no-doc fallback) → then `docs/PRD_scent.md` amendment + `ADR-0004 v2` (approve-before-build), kit `multiplicative_book_v1` vectors as PROPOSED (promote when both implementations reproduce), ScentModel object refactor + cadence policy + belief parameterization + handshake refusal rule + step-0 model-hash sealing + per-model belief-eval rerun.
