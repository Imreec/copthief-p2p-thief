"""Artifact `_schema` prose + links remark — reference shape constants (M6-2).

These strings are part of the artifact interface other tooling reads: they are
EXTRACTED from the reference sample-run fixtures @960499fd (never typed by hand,
never edited here — regenerate from the fixtures on a reference revision) and are
byte-pinned by tests/conformance/test_report_sample_run.py. Attribution: ADR-0002
log (interface-mirrored schema constants, not code).
"""
